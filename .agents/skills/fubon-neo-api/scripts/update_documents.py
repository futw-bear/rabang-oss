#!/usr/bin/env python3
"""Mirror the Fubon Neo API LLM documentation for offline skill use."""
...
DEFAULT_INDEX_URL = "https://www.fbs.com.tw/TradeAPI/llms.txt"
ALLOWED_HOST = "www.fbs.com.tw"
DOCS_PREFIX = "/TradeAPI/docs/"
LINK_PATTERN = re.compile(r"\[[^\]]+\]\((https://www\.fbs\.com\.tw/TradeAPI/docs/[^)\s]+\.txt)\)")
USER_AGENT = "Rabang-Fubon-Neo-Skill-Docs-Updater/1.0"
...
def document_path(url: str) -> Path:
    ...
    if parsed.query or parsed.fragment or not parsed.path.startswith(DOCS_PREFIX):
        raise ValueError(f"Invalid document URL: {url}")

    relative = urllib.parse.unquote(parsed.path[len(DOCS_PREFIX) :])
...
def parse_document_urls(index: str) -> list[str]:
    ...
    if not urls:
        raise RuntimeError("The official index contains no supported document links")
    for url in urls:
        document_path(url)
    return urls
    ...
    try:
        ...
        for item in data.get("documents", []):
            ...
            if not isinstance(raw_path, str) or "\\" in raw_path:
                raise ValueError(f"Unsafe document path in manifest: {raw_path!r}")
...
def main() -> int:
    ...
    try:
        ...
        raise RuntimeError("The official index is not valid UTF-8") from error
    ...
    with ThreadPoolExecutor(max_workers=max(1, args.workers)) as executor:
        jobs = {
            executor.submit(download, url, args.timeout, args.retries): url
            for url in urls
        }
    ...
    for stale in sorted(previous_files - current_files):
        ...
        try:
            ...
            raise ValueError(f"Unsafe stale document path: {stale}") from error
